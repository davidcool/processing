...
public class RhombicTriacontahedron extends Polyhedron {
  ...
  final float C0 = 0.559016994374947424102293417183;  //sqrt(5) / 4
  final float C1 = 0.904508497187473712051146708591;  //(5 + sqrt(5)) / 8
  final float C2 = 1.46352549156242113615344012577;   //(5 + 3 * sqrt(5)) / 8
  private PVector[] vert = new PVector[32]; // list of ve//rtices
  private int[][] faces =  new int[FACES][VERTICES];  // list of faces (joining vertices)
  
  private RhombicTriacontahedron() {   
    // set vertices
    vert[  0] = new PVector(  C1, 0.0,  C2 );
    vert[  1] = new PVector(  C1, 0.0, -C2 );
    vert[  2] = new PVector( -C1, 0.0,  C2 );
    vert[  3] = new PVector( -C1, 0.0, -C2 );
    vert[  4] = new PVector(  C2,  C1, 0.0 );
    vert[  5] = new PVector(  C2, -C1, 0.0 );
    vert[  6] = new PVector( -C2,  C1, 0.0 );
    vert[  7] = new PVector( -C2, -C1, 0.0 );
    vert[  8] = new PVector( 0.0,  C2,  C1 );
    vert[  9] = new PVector( 0.0,  C2, -C1 );
    vert[ 10] = new PVector( 0.0, -C2,  C1 );
    vert[ 11] = new PVector( 0.0, -C2, -C1 );
    vert[ 12] = new PVector( 0.0,  C0,  C2 );
    vert[ 13] = new PVector( 0.0,  C0, -C2 );
    vert[ 14] = new PVector( 0.0, -C0,  C2 );
    vert[ 15] = new PVector( 0.0, -C0, -C2 );
    vert[ 16] = new PVector(  C2, 0.0,  C0 );
    vert[ 17] = new PVector(  C2, 0.0, -C0 );
    vert[ 18] = new PVector( -C2, 0.0,  C0 );
    vert[ 19] = new PVector( -C2, 0.0, -C0 );
  ...
  } //end RhombicTriacontahedron()

  public String name() {
    return "Rhombic Triacontahedron";
  }
...
} //end class RhombicTriacontahedron
