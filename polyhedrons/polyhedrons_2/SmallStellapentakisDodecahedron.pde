/*
SmallStellapentakisDodecahedron class
...
public class SmallStellapentakisDodecahedron extends Polyhedron {
  
  // polyhedron
  final int FACES = 60;     // number of faces 
  final int VERTICES = 3;   // VERTICES per face
  final float C0 = 0.954915028125262879488532914086;  //5 * (3 - sqrt(5)) / 4
  final float C1 = 1.54508497187473712051146708591;   //5 * (sqrt(5) - 1) / 4
  final float C2 = 1.80901699437494742410229341718;   //(5 + sqrt(5)) / 4
  final float C3 = 2.927050983124842272306880251548;  //(5 + 3 * sqrt(5)) / 4
  private PVector[] vert = new PVector[24]; // list of ve//rtices
  private int[][] faces =  new int[FACES][VERTICES];  // list of faces (joining vertices)
  
  private SmallStellapentakisDodecahedron() {   
    // set vertices
  ...
  } //end SmallStellapentakisDodecahedron()

  public String name() {
    return "Small Stellapentakis Dodecahedron";
  }
...
} //end class SmallStellapentakisDodecahedron
