/*
DisdyakisTriacontahedron class
...
public class DisdyakisTriacontahedron extends Polyhedron {
  
  // polyhedron
  final int FACES = 120;     // number of faces 
  final int VERTICES = 3;   // VERTICES per face
  final float C0 = 1.17518645301134929748244365923;   //3 * (15 + sqrt(5)) / 44
  final float C1 = 1.38196601125010515179541316563;   //(5 - sqrt(5)) / 2
  final float C2 = 1.901491624090794379859549273853;  //3 * (5 + 4 * sqrt(5)) / 22
  final float C3 = 2.17082039324993690892275210062;   //3 * (5 + sqrt(5)) / 10
  final float C4 = 2.23606797749978969640917366873;   //sqrt(5)
  final float C5 = 3.07667807710214367734199293309;   //(75 + 27 * sqrt(5)) / 44
  final float C6 = 3.51246117974981072676825630186;   //(15 + 9 * sqrt(5)) / 10
  final float C7 = 3.61803398874989484820458683437;   //(5 + sqrt(5)) / 2
  final float C8 = 3.80298324818158875971909854771;   //3 * (5 + 4 * sqrt(5)) / 11
  private PVector[] vert = new PVector[62]; // list of ve//rtices
  private int[][] faces =  new int[FACES][VERTICES];  // list of faces (joining vertices)
  
  private DisdyakisTriacontahedron() {   
    // set vertices
  ...
  } //end DisdyakisTriacontahedron()

  public String name() {
    return "Disdyakis Triacontahedron";
  }
...
} //end class DisdyakisTriacontahedron
