...
public class PentakisDodecahedron extends Polyhedron {
  
  // polyhedron
  final int FACES = 60;     // number of faces 
  final int VERTICES = 3;   // VERTICES per face
  final float C0 = 0.927050983124842272306880251548;  //3 * (sqrt(5) - 1) / 4
  final float C1 = 1.33058699733550141141687582919;   //9 * (9 + sqrt(5)) / 76
  final float C2 = 2.15293498667750705708437914596;   //9 * (7 + 5 * sqrt(5)) / 76
  ...
  private PentakisDodecahedron() {   
    ...
    vert[ 12] = new PVector(   C1,  0.0,   C2 );
    vert[ 13] = new PVector(   C1,  0.0,  -C2 );
    vert[ 14] = new PVector(  -C1,  0.0,   C2 );
    vert[ 15] = new PVector(  -C1,  0.0,  -C2 );
    vert[ 16] = new PVector(   C2,   C1,  0.0 );
    vert[ 17] = new PVector(   C2,  -C1,  0.0 );
    vert[ 18] = new PVector(  -C2,   C1,  0.0 );
    vert[ 19] = new PVector(  -C2,  -C1,  0.0 );
    vert[ 20] = new PVector(  0.0,   C2,   C1 );
    vert[ 21] = new PVector(  0.0,   C2,  -C1 );
    vert[ 22] = new PVector(  0.0,  -C2,   C1 );
    vert[ 23] = new PVector(  0.0,  -C2,  -C1 );
  ...
  } //end PentakisDodecahedron()

  public String name() {
    return "Pentakis Dodecahedron";
  }
...
} //end class PentakisDodecahedron
