...
public class TriakisIcosahedron extends Polyhedron {
  
  // polyhedron
  final int FACES = 60;     // number of faces 
  final int VERTICES = 3;   // VERTICES per face
  final float C0 = 1.04955317926133973822831518963;   //5 * (7 + sqrt(5)) / 44
  final float C1 = 1.69821271704535895291326075851;   //5 * (3 + 2 * sqrt(5)) / 22
  final float C2 = 1.80901699437494742410229341718;   //(5 + sqrt(5)) / 4
  final float C3 = 2.74776589630669869114157594814;   //5 * (13 + 5 * sqrt(5)) / 44
  ...
  } //end TriakisIcosahedron()

  public String name() {
    return "Triakis Icosahedron";
  }
