...
public class polyhedrons_5 extends PApplet {
...
public void draw() {
  ...
  for (Sphere s: spheres) {
    pushMatrix();
    s.update(r + PApplet.parseInt(fft.getBand(i) * 2) );
    s.render();
    popMatrix();
    i++;
  }
...
public void mouseWheel(MouseEvent e) {
  zoom += map(e.getCount(), -10, 10, 0.001f, -0.001f);
  zoom = constrain(zoom, 0.00001f, 1.5f);
};
...
class Sphere {
  ...
  public void addSphereItem() {
    Polyhedron si = new GreatIcosihemidodecahedron();
    //Set random values for the spherical coordinates
    si.positionSphere(radius, random(PI * 2), random(PI * 2));
    //scale
    si.scaleFactor(5000);
    //fill
    si.fillColor(PApplet.parseInt(random(255)), PApplet.parseInt(random(255)), PApplet.parseInt(random(255)), PApplet.parseInt(random(100)));
    //stroke
  ...
  public void addSphereItem(String _type) {
    ...
    if (type.equals("SmallHexagrammicHexecontahedron") == true) {
      Polyhedron si = new GreatSnubIcosidodecahedron();
      //Set random values for the spherical coordinates
      si.positionSphere(radius, random(PI * 2), random(PI * 2));
      //scale
      si.scaleFactor(5000);
  ...
  public void update(int _radius) {
    radius = _radius;
    for (Polyhedron p: items) {
      //p.radius = radius + int(fft.getBand(i) * 0.25);
      p.update();
    };
  };
  ...
  public void render() {
    ...
    for (Polyhedron p: items) {
      p.scaleFactor(PApplet.parseInt(radius + fft.getBand(i) * 0.01f));
      p.speed( fft.getBand(i) * 0.00000003f, fft.getBand(i) * 0.00800005f );
      p.rotate(radians(random(-0.08f,0.08f)-frameCount%360), radians(random(-0.05f,0.05f)+frameCount%360), 0);
      p.render();
    };
...
public class TruncatedTetrahedron extends Polyhedron {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "polyhedrons_5" };
    if (passedArgs != null) {
